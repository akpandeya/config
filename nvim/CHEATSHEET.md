...
| `<leader>fg` (`Space + f + g`) | **Grep Files** | Prompt for a string; list each matching file once (open it, then `/` to the match) |
| `<leader>fG` (`Space + f + G`) | **Live Grep** | Search for text across all files, one entry per matching line |
