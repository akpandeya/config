...
Conventions (jarvis-specific): tiers `S/A/B/C/D`, milestones `Wave 1…5`, `Blocked by: #N, #M` as the first line of each body.
...
2. Build a map (scratch file is fine) of:
     ...
     - milestone consistency (Wave 1 blocked by Wave 3 is a bug)
...
3. For each anomaly, propose a fix and ask before applying:
     ...
     - tier/milestone mismatch with dependencies → propose retier/remilestone
...
4. Holistic tier reassignment. Once bodies are clean and the graph is sane, re-evaluate tiers across the *whole* backlog — not just where dependencies disagreed. With full context now available, the existing tier labels are stale priors; treat each issue's tier as something to re-derive, not defend.
     - Re-read the tier rubric in `CLAUDE.md` ("Three-axis backlog scheme"):
...
6. End with a short report: edges added, edges removed, dups closed, tier reassignments applied, milestone moves, remaining smells you didn't auto-fix.
